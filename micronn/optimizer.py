...
class Optimizer:
    ...
    def init_optim_grad(self, grad, optim_grad):
        for i in list(grad.keys()):
            if 'dW' in i or 'db' in i:
                optim_grad[i] = np.zeros((grad[i].shape[0], grad[i].shape[1]))
        return optim_grad
...
class GD(Optimizer):
    ...
    def update(self, param, grad, L):
        self.v = self.init_optim_grad(grad, self.v)
...
class RMSprop(Optimizer):
    ...
    def update(self, param, grad, L):
        self.s = self.init_optim_grad(grad, self.s)
